...
exports.LGT_TRADE = {
    ...
    Log: () => {
        ...
        Logger.talk(
            `Max TX Cost: ${ethers.utils.formatEther(
                (
                    this.LGT_TRADE.GasLimitForTrade *
                    this.LGT_TRADE.GasPriceForTrade
                ).toString()
            )}`
        );
    },
...
exports.CHI_TRADE = {
    ...
    Log: () => {
        ...
        Logger.talk(
            `Max TX Cost: ${ethers.utils.formatEther(
                (
                    this.CHI_TRADE.GasLimitForTrade *
                    this.CHI_TRADE.GasPriceForTrade
                ).toString()
            )}`
        );
    },
...
const updateLgt = async () => {
    ...
    this.LGT_TRADE.Readable.GasPrice = `${this.LGT_TRADE.GasPriceForTrade /
        1000000000} Gwei`;

    this.LGT_TRADE.GasLimitForTrade =
        (39141 + 36224 + 55000) * this.LGT_TRADE.BuyAmount; // overhead * amount

    const tradeInfo = await ShadowGas.lgtTradeInfo(
        parseInt(this.LGT_TRADE.BuyAmount),
        {
    ...
    this.LGT_TRADE.Readable.GasCost = `Gas Cost: ${ethers.utils.formatEther(
        tradeInfo[1].toString()
    )} Eth`;

    this.LGT_TRADE.ExpectedProfit = tradeInfo[0];
    this.LGT_TRADE.Readable.Profit = `Expected Profit: ${ethers.utils.formatEther(
        tradeInfo[0].toString()
    )} Eth`;
...
const updateChi = async () => {
    ...
    this.CHI_TRADE.Readable.GasPrice = `${this.CHI_TRADE.GasPriceForTrade /
        1000000000} Gwei`;

    this.CHI_TRADE.GasLimitForTrade =
        39141 + 36224 + 55000 * this.CHI_TRADE.BuyAmount; // overhead * amount

    this.CHI_TRADE.GasCostForTrade = ethers.BigNumber.from(
        (
            (39141 + 36224 * this.CHI_TRADE.BuyAmount + 55000) *
            this.CHI_TRADE.GasPriceForTrade
        ).toString()
    );

    this.CHI_TRADE.Readable.GasCost = `Gas Cost: ${ethers.utils.formatEther(
        this.CHI_TRADE.GasCostForTrade.toString()
    )} Eth`; // Mint

    if (this.CHI_TRADE.ExpectedProfit > this.CHI_TRADE.GasCostForTrade)
        this.CHI_TRADE.Profitable = true;
...
const oneInchProfit = async () => {
    ...
    const request = await fetch(
        `https://api.1inch.exchange/v1.1/quote?fromTokenSymbol=CHI&toTokenSymbol=ETH&amount=${amountIn}`
    );
...
const uniswapv2ChiProfit = async (withSlippage) => {
    const CHI = new uniswap.Token(
        uniswap.ChainId.MAINNET,
        GasToken.chi.address,
        GasToken.chi.decimals
    );

    const pair = await uniswap.Fetcher.fetchPairData(
        CHI,
        uniswap.WETH[CHI.chainId]
    );
    ...
    return withSlippage
        ? trade.minimumAmountOut(slippage).toSignificant(9)
        : trade.outputAmount.toSignificant(9);
};
