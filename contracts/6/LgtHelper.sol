...
contract LgtHelper {
    ...
    function burnAndFree(uint256 burn, uint256 tokenAmount)
        public
        returns (bool success)
    {
        burnGas(burn);
    ...
    function burnAndFreeGST(uint256 burn, uint256 tokenAmount)
        public
        returns (bool success)
    {
        burnGas(burn);
    ...
    function burnAndFreeCHI(uint256 burn, uint256 tokenAmount)
        public
        returns (uint256)
    {
        burnGas(burn);
    ...
    function burnBuyAndFreeOpt(uint256 burn, uint256 tokenAmount)
        public
        payable
    {
        burnGas(burn);
